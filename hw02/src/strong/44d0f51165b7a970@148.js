...
function _5(Counts,constellations,data)
{
...
function _transformedData(Counts,constellationMapping){return(
Counts.map(item => ({
    ...item,
...
md`<h1>以 bar chart 呈現每個星座的人數</h1>`
)}
...
Plot.plot({
  ...
  color: {
    domain: ['male', 'female'],
    range: ['#58B2DC', '#F596AA'], //可以根據需要更改顏色
    legend: true
  },
  marks: [
...
md`<h1>以 histogram 呈現每個星座的人數</h1>`
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["Counts","constellations","data"], _5);
  main.variable(observer("constellationMapping")).define("constellationMapping", _constellationMapping);
  main.variable(observer("transformedData")).define("transformedData", ["Counts","constellationMapping"], _transformedData);

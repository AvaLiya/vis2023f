...
function _Counts(yCounts,years,data)
{
...
function _6(Plot,Counts){return(
Plot.plot({
  grid: true,
  y: {label: "count"},
  marks: [
    Plot.ruleY([0]),
    Plot.barY(Counts, {x: "year", y: "count"}),
  ]
})
)}
...
function _8(Plot,plot2,Counts){return(
Plot.plot({
  marginTop: plot2.mt,
  ...
  color: {
    domain: ['male', 'female'],
    range: ['#58B2DC', '#F596AA'],  // 你可以根據需要更改顏色
    legend: true
  },
  marks: [
    Plot.ruleY([0]),
    Plot.barY(Counts, {x: "year", y: "count", tip: true , fill:"gender"}),
  ]
})
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("Counts")).define("Counts", ["yCounts","years","data"], _Counts);
  main.variable(observer()).define(["Plot","Counts"], _6);
  ...
  main.variable(observer()).define(["Plot","plot2","Counts"], _8);
  return main;
}
